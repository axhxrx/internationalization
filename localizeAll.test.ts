...
{
  const t = localizeAll<AppLocales, typeof translations>(
    translations,
    { locale: 'en' },
  );
  ...
  // Test German locale
  const tDe = localizeAll<AppLocales, typeof translations>(
    translations,
    { locale: 'de' },
  );
...
Deno.test('localizeAll - functions with single parameter', () =>
{
  ...
  assertEquals(t.button.delete('Untitled Document'), 'Delete Untitled Document');
  assertEquals(t.message.hello('Alice'), 'Hello, Alice!');

  // Test Japanese locale
  const tJa = localizeAll<AppLocales, typeof translations>(
    translations,
    { locale: 'ja' },
  );
...
Deno.test('localizeAll - functions with optional parameters', () =>
{
  ...
  assertEquals(t.message.itemsFound(3, 'typescript'), 'Found 3 items matching "typescript"');
